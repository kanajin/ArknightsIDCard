...
class BKG:
    ...
    def bkg_init(self):
        ...
        for numList in range(10, self.num):
            ...
            outHeight = 206*Line + int(self.data['Gap'])*(Line-1) + int(self.data['TopSide']) + 180 + int(self.data["FirstLine"])
            outWide = 180*numList + int(self.data['Gap'])*(numList-1) + int(self.data['LeftSide']) + int(self.data['RightSide'])
    ...
    def addtitle(self) -> None:
        ...
        if OnlyEn:                # DrName font setting of if only [A-Za-z0-9]
            fontName = ImageFont.truetype("./Source/TTF/AcuminProBook.ttf", int(self.data['SizeName']))
        else:                       # DrName font setting of other situation
            fontName = ImageFont.truetype("./Source/TTF/NotoSansHans-Regular.otf", int(self.data['SizeName'])-5)
        fontDayAndID = ImageFont.truetype("./Source/TTF/AcuminProBook.ttf", int(self.data['SizeID']))  # date and ID
        fontTotal = ImageFont.truetype("./Source/TTF/BenderRegular.otf", int(self.data['SizeTotal']))  # total of skill-elite 3
        ...
        self.img.paste(skillElite3Big, (self.img.size[0] - int(self.data['TotalToRight']) - skillElite3Big.size[0] - 50,
                                        int(self.data['TopSide'])), mask=a)  # 粘贴
        New = Image.open(pathHead).convert("RGBA")  # 获取头像
        a = New.split()[3]
        self.img.paste(New, (int(self.data['LeftSide']), int(self.data['TopSide'])), mask=a)  # 粘贴头像
        New = Image.open(pathHeadBlock).convert("RGBA")  # 获取头像框
        a = New.split()[3]
        self.img.paste(New, (int(self.data['LeftSide']) - 3, int(self.data['TopSide']) - 3), mask=a)  # 粘贴头像框
        titleDraw = ImageDraw.Draw(self.img)  # add name
        text = "Dr. " + self.data['Name']
        titleDraw.text((int(self.data['LeftSide']) + 220, int(self.data['TopSide']) - 30 + OnlyEn*EnBig), text, font=fontName)
        text = self.data['DateIn'] + "——" + self.data['DateNow'] + "     ID:" + self.data['ID']
        titleDraw.text(((int(self.data['LeftSide']) + 228, int(self.data['TopSide']) + 137)), text, font=fontDayAndID)  # add date
        global NumSkillThr
        text = str(NumSkillThr)
        titleDraw.text((self.img.size[0] - int(self.data['TotalToRight']), int(self.data['TopSide']) + 10), text,
...
class GanYuan:
    ...
    def add_skill(self) -> None:
        ...
        PathOri = "./Source/Character/头像_" + self.data['Operator'] + skinDict[self.data['Illustration']]
        ...
        for num in range(3):      # for skill1,2,3
            if Skill[num] == 0:
                continue
            if (int(Skill[num]) < 7) or (int(self.data['Stage']) != 2):
        ...
        self.imgPath = "./OutPut/头像_" + self.data['Operator'] + skinDict[self.data['Illustration']]
        editing.save(self.imgPath)  # save

    def add_star(self) -> None:
        editing = Image.open(self.imgPath)
        pathLevel = "./Source/Star/BKG"+self.data['Star']+"star.png"
    ...
    def add_pot(self) -> None:  # potential
        editing = Image.open(self.imgPath)
        PathPot = "./Source/Potential/" + self.data['Potential'] + ".png"
    ...
    def add_level(self) -> None:  # level
        editing = Image.open(self.imgPath)
        if int(self.data['Stage']) != 2:
            PathElite = "./Source/Elite/" + self.data['Stage'] + ".png"
    ...
    def add_mod(self) -> None:   # cover mod and mod level
        ...
        stdModSize = 42
        if not self.data['Modules'] == '':    # if Mod exist
            ...
            # paste mod light
            PathMod = "./Source/Mod/模组类型_" + self.data['Modules'] + ".png"
            ...
            # 以模组背景为基准居中粘贴
            pathModLevel = "./Source/Mod/" + self.data["Modules Stage"] + ".png"
...
def draw():
    ...
    for i in range(len(BoxData)):
        ...
        if (i % DrBKG.ShowList) == 0:  # 计算小头像所在行
            ShowLine = ShowLine + 1
        ShowX = int(DrBKG.data['LeftSide']) + (i % DrBKG.ShowList) * (180+int(DrBKG.data['Gap']))  # 小头像横坐标
        ShowY = int(DrBKG.data['TopSide']) + ShowLine * (206+int(DrBKG.data['Gap'])) + int(DrBKG.data['FirstLine']) - 26  # 小头像纵坐标
